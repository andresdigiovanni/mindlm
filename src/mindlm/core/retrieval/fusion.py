...
def _rrf_merge(ranked_lists: list[list[Result]], k: int = 60) -> list[Result]:
...
class FusionEngine:
    ...
    def fuse(
        ...
        Args:
            ...
            top_k: Number of candidates to fetch per query and final result count
        ...
        ranked_lists = [
            self._retriever.retrieve(q, filters, top_k=top_k) for q in queries
        ]
        merged = _rrf_merge(ranked_lists, k=self._rrf_k)[:top_k]
        langfuse_context.update_current_observation(
            metadata={"queries_count": len(queries), "merged_count": len(merged)},
        )
        return merged
